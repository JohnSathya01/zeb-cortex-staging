...
export default function ReviewChatsPage() {
  const { user } = useAuth();
  ...
  return (
    <div>
      <div className="page-header"><h1>Review Chats</h1></div>
      <div className="chat-list-page">
        <div className="chat-conversations">
          ...
          {conversations.length === 0 ? (
            ...
            <button key={c.assignmentId} className={`chat-conversation-item ${selectedId === c.assignmentId ? 'active' : ''}`} onClick={() => setSelectedId(c.assignmentId)}>
        ...
        <div className="chat-thread">
          {selected ? (
            <ChatPanel messages={messages} currentUserId={user.uid} onSend={(text) => sendChatMessage(selectedId, text)} headerLabel={`Chat with ${selected.learnerName}`} />
          ) : (
